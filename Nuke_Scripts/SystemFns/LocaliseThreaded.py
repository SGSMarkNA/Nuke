...
class LocaliseThreaded(object):
	...
	def reportFinishedThread(self):
		...
		if self.finishedThreads == self.taskCount:
			...
			#### Workaround to make ProgressTask progress bar go away when it's done...
			del(self.mainTask)            
...
def localiseFileThreaded(readKnobList):
	...
	for knob in readKnobList:
		...
		#try:
			...
			#filePathListProxy = getFrameList(proxyKnob, allFilesPaths)
			#print 'proxy files:', filePathListProxy
			#filePathList.extend(filePathListProxy)
		#except:
			#logger.warn('no proxy knob found for {0}'.format(knob.node().name()))
			#pass
